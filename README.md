...
Run `phite dev` from a PHP Project containing `public/index.php` or `index.php`. Phite starts FrankenPHP in classic mode on an available `127.0.0.1` port and prints the Local Endpoint after it is ready. It serves existing static files directly and sends other requests to the Entrypoint. The browser stays closed unless you press `o` then Enter, and Ctrl+C stops the Development Session. Starting and stopping does not rewrite Project files.
